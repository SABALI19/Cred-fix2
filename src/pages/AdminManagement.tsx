      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto space-y-6 px-4 py-6 sm:py-8">
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            ...
            <Button className="w-full md:w-auto" variant="outline" onClick={() => Promise.all([loadUsers(), loadOperations(), loadPlatformData()])}>
              Refresh Data
            </Button>
          </div>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
            <div className="-mx-1 overflow-x-auto pb-1">
              <TabsList className="inline-flex h-auto min-w-max gap-1 px-1">
                ...
                <div className="grid gap-6 xl:grid-cols-[1.2fr,1fr]">
                  <Card>
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <Button className="w-full sm:w-auto" onClick={saveUser}>Save User Changes</Button>
                    </CardContent>
                  </Card>
            ...
            <TabsContent value="platform" className="space-y-6">
              ...
              {platformControls ? (
                <Card><CardHeader><CardTitle>Platform Controls</CardTitle></CardHeader><CardContent className="space-y-6"><div className="space-y-3"><h3 className="font-medium">Charts</h3>{Object.entries(platformControls.charts).map(([key, value]) => <div key={key} className="flex flex-col gap-3 rounded border p-3 sm:flex-row sm:items-center sm:justify-between"><span>{key}</span><Switch checked={value.enabled} onCheckedChange={(checked) => setPlatformControls((prev) => prev ? { ...prev, charts: { ...prev.charts, [key]: { enabled: checked } } } : prev)} /></div>)}</div><div className="space-y-4"><h3 className="font-medium">Service Cards</h3>{Object.entries(platformControls.services).map(([key, value]) => <div key={key} className="space-y-3 rounded border p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><span className="font-medium">{key}</span><Switch checked={value.enabled} onCheckedChange={(checked) => setPlatformControls((prev) => prev ? { ...prev, services: { ...prev.services, [key]: { ...prev.services[key as keyof PlatformControls["services"]], enabled: checked } } } : prev)} /></div><Input value={value.title} onChange={(e) => setPlatformControls((prev) => prev ? { ...prev, services: { ...prev.services, [key]: { ...prev.services[key as keyof PlatformControls["services"]], title: e.target.value } } } : prev)} placeholder="Title" /><Textarea value={value.description} onChange={(e) => setPlatformControls((prev) => prev ? { ...prev, services: { ...prev.services, [key]: { ...prev.services[key as keyof PlatformControls["services"]], description: e.target.value } } } : prev)} /><Input value={value.ctaLabel} onChange={(e) => setPlatformControls((prev) => prev ? { ...prev, services: { ...prev.services, [key]: { ...prev.services[key as keyof PlatformControls["services"]], ctaLabel: e.target.value } } } : prev)} placeholder="CTA label" /></div>)}</div><Button className="w-full sm:w-auto" onClick={savePlatformControls}>Save Platform Controls</Button></CardContent></Card>
              ) : null}
            </TabsContent>
